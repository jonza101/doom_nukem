/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   draw.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: zjeyne-l <zjeyne-l@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/27 13:56:20 by zjeyne-l          #+#    #+#             */
/*   Updated: 2019/07/28 19:11:12 by zjeyne-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "doom_nukem.h"

void	ft_gun_anim(t_mlx *mlx, t_weapon *gun, int delay)
{
	if (mlx->player->weapon_state == 1)
	{
		mlx->gun_delay++;
		if (mlx->gun_fire_i < REV_FIRE && mlx->gun_delay == delay)
		{
			mlx->player->frame = gun->fire[mlx->gun_fire_i];
			mlx->gun_fire_i++;
			mlx->gun_delay = 0;
		}
		else if (mlx->gun_fire_i == REV_FIRE)
		{
			mlx->player->weapon_state = 0;
			mlx->gun_fire_i = 0;
		}
	}
}

void	ft_weapon_state(t_mlx *mlx)
{
	if (mlx->player->weapon_state == 0)
		mlx->player->frame = mlx->player->revolver->idle[0];
}

void	ft_draw_player(t_mlx *mlx)
{
	double w_aspect_ratio = (double)mlx->player->frame->h / (double)mlx->player->frame->w;
	double w_ya = (double)(H / 2.0f);// + ((double)H * (double)(W / H) / 30.0f);
	double w_yb = H;
	double w_h = w_yb - w_ya;
	double w_w = w_h / (double)w_aspect_ratio;
	// ft_image(mlx, W / 2, w_ya, 0xFFFFF);
	// ft_image(mlx, W / 2 - (w_w / 2), w_ya, 0xFFFFF);
	// ft_image(mlx, W / 2 + (w_w / 2), w_ya, 0xFFFFF);
	int x = -1;
	while (++x < w_w)
	{
		int y = -1;
		while (++y < w_h)
		{
			double sample_x = x / w_w;
			double sample_y = y / w_h;
			int color = ft_texture_sampling(mlx->player->frame, sample_x, sample_y);
			int xc = (int)(W / 2 - (w_w / 2.0f) + x);
			int yc = (int)w_ya + (int)y;
			if (color != IGNORE_COLOR)
				ft_image(mlx, xc, yc, color);
		}
	}
}
